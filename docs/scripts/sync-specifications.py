#!/usr/bin/env python3
"""Validate specifications and regenerate one status-grouped index per type."""
...
ID = re.compile(r"(?:ADR|UC|FR|NFR|PLAN)-[0-9]{6}\Z")
FILENAME = re.compile(r"((?:ADR|UC|FR|NFR)-[0-9]{6})-[a-z0-9]+(?:-[a-z0-9]+)*\.md\Z")
...
def validate(path, data, kind):
    ...
    if status not in states(kind):
        errors.append("status must be one of: " + ", ".join(states(kind)))
...
def relation_errors(records, catalog):
    ...
    for path, data, kind in records:
        ...
        if data.get("status") == "superseded" and isinstance(identifier, str):
            ...
            if not any(d.get("status") in (TYPES[k][3], "superseded") and
...
def index_content(kind, records, directory):
    title = TYPES[kind][2]
    back = "../" * len(Path(TYPES[kind][1]).parts) + "README.md"
    lines = [f"# {title}", "", "<!-- Generated by docs/scripts/sync-specifications.py. Do not edit manually. -->", "",
             f"[Documentation]({back})", ""]
    ordered = sorted((r for r in records if r[2] == kind), key=lambda r: r[1]["id"])
    for status in states(kind):
        ...
        for path, data, _ in matching:
            ...
            lines.append(f"| {data['id']} | [{table_text(data['title'])}]({href}) | {data['revision']} | {iso_date(data['updated']).isoformat()} |")
        if not matching:
...
def synchronize(root, apply=False, check=False):
    ...
    for kind, (_, folder, _, _) in TYPES.items():
        ...
        for path in sorted(directory.rglob("*.md")):
            ...
            errors.extend(f"{path.relative_to(root).as_posix()}: {error}" for error in validate(path, data, kind))
    ...
    for path in sorted((root / "docs/plans").rglob("*.md")):
        ...
        if not path.resolve().is_relative_to(root):
            errors.append(f"{path}: plan resolves outside repository")
            continue
        try:
            data = read_metadata(path)
            identifier = data.get("id")
            if not isinstance(identifier, str) or not re.fullmatch(r"PLAN-[0-9]{6}", identifier):
                errors.append(f"{path}: invalid plan ID")
            else:
                catalog[identifier].append((path, data, "plan"))
        except (ValueError, yaml.YAMLError, OSError, UnicodeError) as exc:
            errors.append(f"{path}: {exc}")
    for identifier, matches in catalog.items():
        if len(matches) > 1:
            errors.append(f"duplicate ID {identifier}: " + ", ".join(str(p) for p, _, _ in matches))
    errors.extend(relation_errors(records, catalog))
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
    ...
    for kind, (_, folder, _, _) in TYPES.items():
        ...
        content = index_content(kind, records, directory)
