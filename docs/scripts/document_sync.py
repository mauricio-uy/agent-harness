...
ID = re.compile(r"(?:ADR|UC|FR|NFR|RES|PLAN)-[0-9]{6}\Z")
FILENAME = re.compile(r"((?:ADR|UC|FR|NFR|RES)-[0-9]{6})-[a-z0-9]+(?:-[a-z0-9]+)*\.md\Z")
...
def validate(path, data, kind, types):
    errors = []
    ...
    if status not in states(kind, types):
        errors.append("status must be one of: " + ", ".join(states(kind, types)))
...
def index_content(kind, records, directory, types, generator):
    title = types[kind][2]
    back = "../" * len(Path(types[kind][1]).parts) + "README.md"
    lines = [f"# {title}", "", f"<!-- Generated by docs/scripts/{generator}. Do not edit manually. -->", "",
             f"[Documentation]({back})", ""]
    ordered = sorted((r for r in records if r[2] == kind), key=lambda r: r[1]["id"])
    for status in states(kind, types):
...
def synchronize(root, types, generator, apply=False, check=False):
    ...
    for kind, (_, folder, _, _) in types.items():
        ...
        for path in sorted(directory.rglob("*.md")):
            ...
            errors.extend(f"{path.relative_to(root).as_posix()}: {error}" for error in validate(path, data, kind, types))
    ...
    for kind, (_, folder, _, _) in types.items():
        ...
        content = index_content(kind, records, directory, types, generator)
...
def run(types, generator, description):
