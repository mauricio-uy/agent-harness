...
Links point from the outside in: entry points (`README.md`, `AGENTS.md`, this file) link to skills, a skill links to its own references, and a reference links to its own templates. A skill may also link into `docs/`, where the artifacts it operates on live, and to [.agents/shared/](../.agents/shared/README.md). A skill may link to another skill's format in `references/` when it reads or modifies that skill's artifact, as `implement-plan` does with the plan format; it never links to another skill's templates or other internals. Project documents under `docs/` do not link into `.agents/`, so a project can replace or remove the harness without breaking them; this file is the exception, because it documents the harness itself.
