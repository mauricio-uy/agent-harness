...
2. Open only the selected type's template and the index of the state you need. Manage its directory and index as defined in the reference. Search for overlapping documents before creating or revising one. Get a new ID with `harness id <type>` and dates with `harness date`. Preserve confirmed facts, distinguish proposals from decisions, and ask about missing information that affects meaning or acceptance.
