...
6. After each step, run its completion check and update the plan's execution record, taking dates from `harness date`. Keep RED/GREEN evidence concise and factual. Before pausing, record the exact stopping point, partial work, failing or pending checks, and what is needed to resume. Never mark a partial step complete or remove the history of a blocker.
