...
1. Identify the operation or incident, intended operator, target environment, and known procedure. Search existing runbooks before creating a record. Get a new ID with `harness id runbook` and dates with `harness date`. Clarify missing information that changes applicability, permissions, or recovery.
