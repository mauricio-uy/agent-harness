...
class StagedDocumentationTests(unittest.TestCase):
    def setUp(self):
        ...
        shutil.copytree(SCRIPTS, self.root / ".agents/harness/scripts", ignore=shutil.ignore_patterns("__pycache__", "tests"))
        (self.root / ".agents/harness/scripts/README.md").unlink()
        (self.root / ".agents/harness/scripts/guide.md").unlink()
