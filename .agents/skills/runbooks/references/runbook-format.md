...
Update the affected rows directly when creating or revising runbooks; no runbook synchronization script exists yet. Preserve unrelated entries and navigation. Do not use specification or research synchronization to manage this index. Read only relevant sections or search by ID. The index links back to `docs/README.md` and contains no skill instructions.
