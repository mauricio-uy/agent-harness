...
Regenerate the index from frontmatter with `sync-runbooks.py` after creating or revising runbooks; do not edit generated rows. Keep custom navigation in `docs/README.md`. Do not use specification or research synchronization to manage this index. Read only relevant sections or search by ID. The index links back to `docs/README.md` and contains no skill instructions.
