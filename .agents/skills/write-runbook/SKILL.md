...
6. Use `sync-runbooks.py` to validate runbook metadata and regenerate only its index, then check links. Follow [the documentation commands](../../../.agents/harness/scripts/guide.md#synchronize-runbooks) for preview, apply, and read-only checks. Report references requiring changes outside this skill's scope.
