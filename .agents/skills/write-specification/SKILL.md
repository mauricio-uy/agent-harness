...
4. Check consistency with related documents and verify referenced IDs. When `docs/glossary.md` is present, use its terms and raise any conflicting or new term for human review. Apply the revision and replacement rules from the reference. Present the document's ID, type, revision, and pending decisions for human review; resolve blockers before setting `awaiting-approval`.
...
6. Validate metadata and regenerate the type indexes with `sync-specifications.py`; check local links and repair reported references. Follow [the script commands](../../../.agents/harness/scripts/guide.md) for preview, apply, and read-only checks. Edit document frontmatter, not generated rows.
