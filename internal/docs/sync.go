...
// DocType describes one document type: its ID prefix, directory under docs/,
// index title, and the state that records approval.
type DocType struct {
	Kind, Prefix, Folder, Title, Approved string
}
...
var Runbooks = &Suite{
	Name:     "runbooks",
	Types:    []DocType{{"runbook", "RUN", "runbooks", "Runbooks", "approved"}},
	states:   func(DocType) []string { return runbookStates },
...
func (s *Suite) statesFor(t DocType) []string {
	if s.states != nil {
		return s.states(t)
	}
	return []string{"awaiting-approval", "draft", t.Approved, "rejected", "superseded"}
}
...
func (s *Suite) indexContent(t DocType, records []record, directory string) string {
	var ordered []record
	for _, r := range records {
		if r.kind == t.Kind {
	...
	for _, status := range s.statesFor(t) {
		lines = append(lines, "## "+titleCase(status), "",
...
// Sync validates the suite's documents and regenerates its indexes. It writes
// only with apply; with check it fails when an index is out of date. Any
// validation error aborts before the first write.
func (s *Suite) Sync(root string, apply, check bool, out io.Writer) int {
	...
	for _, t := range s.Types {
		...
		if !within(root, directory) || isFile(directory) {
			errors = append(errors, relSlash(root, directory)+": invalid document directory")
			continue
		}
		...
		for _, path := range markdownFiles(directory) {
			...
			if isSymlink(path) || !within(directory, path) {
				errors = append(errors, rel+": document must be a regular file within its type directory")
				continue
			}
			...
			r := record{path, rel, data, t.Kind}
			records = append(records, r)
			validate := func(r record, t DocType, s *Suite) []string { return validateDocument(r, t, s.statesFor(t)) }
			if s.validate != nil {
				validate = s.validate
			}
	...
	for _, t := range s.Types {
		...
		content := s.indexContent(t, records, directory)
